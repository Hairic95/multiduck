...
func _create_match_make_request(
	...
	if response.get('code') != null:
		...
		return
	var room = ColyseusRoom.new(response["name"], schema_type)
	room.room_id = response["roomId"]
	room.session_id = response["sessionId"]
	
	room.connect_remote(_build_endpoint(response, { "sessionId": room.session_id }))
...
func _process_rooms(result, promise: Promise):
	var list = []
	for data in result:
		list.append(ColyseusRoomInfo.new(data))
	return list
