extends ColyseusSchemaInterface
...
func _get_property_list():
	...
	for field in _fields:
		result.append({
			name = field.name,
			type = ColyseusTypes.to_gd_type(field.current_type.type),
...
func _setup_field(field: Field):
	...
	match type.type:
		...
		ColyseusTypes.REF:
			assert(type.sub_type != null) #,"Schema type is requested")
		ColyseusTypes.NUMBER, ColyseusTypes.FLOAT32, ColyseusTypes.FLOAT64:
			field.value = 0.0
		ColyseusTypes.INT8, ColyseusTypes.UINT8, ColyseusTypes.INT16, ColyseusTypes.UINT16, ColyseusTypes.INT32, ColyseusTypes.UINT32, ColyseusTypes.INT64, ColyseusTypes.UINT64:
			field.value = 0
...
func decode(decoder: Decoder) -> int:
	...
	var ref_id = 0
	var ref: Ref = Ref.new(self, TypeInfo.new(ColyseusTypes.REF))
	_refs[ref_id] = ref
	var changes = []
	var changed_objects = {}
	...
	while decoder.has_more():
		...
		var is_schema = ref.type_info.type == ColyseusTypes.REF
		...
		if ref_value is SchemaInterface:
			...
			var new
			var key = field_index 
			if ref.type_info.type != ColyseusTypes.MAP:
